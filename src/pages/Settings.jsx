        ...
        <div className="grid gap-8">
            ...
            <section className={`p-8 rounded-[3rem] border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 shadow-2xl-blue' : 'bg-white border-slate-200 shadow-xl'}`}>
                <div className="flex items-center gap-4 mb-8">
                    <div className={`p-3 rounded-2xl ${theme === 'dark' ? 'bg-white/10 text-yellow-400' : 'bg-green-50 text-green-600'}`}>
                        <Layout size={24} />
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <button 
                        ...
                        className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-4 relative overflow-hidden ${theme === 'light' ? 'border-green-600 bg-green-50/50' : 'border-slate-100 opacity-60 hover:opacity-100'}`}
                    >
                        <Sun size={32} className={theme === 'light' ? 'text-green-600' : 'text-slate-400'} />
                        <span className="font-black text-sm uppercase tracking-widest text-slate-800">{t('light_mode')}</span>
                        {theme === 'light' && <div className="absolute top-2 right-2 p-1 bg-green-600 text-white rounded-full"><Check size={10} /></div>}
                    </button>

                    <button 
                        onClick={() => setTheme('dark')}
                        className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-4 relative overflow-hidden ${theme === 'dark' ? 'border-green-500 bg-slate-800' : 'border-slate-100 opacity-60 hover:opacity-100'}`}
                    >
                        <Moon size={32} className={theme === 'dark' ? 'text-yellow-400' : 'text-slate-400'} />
                        <span className={`font-black text-sm uppercase tracking-widest ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>{t('dark_mode')}</span>
                        {theme === 'dark' && <div className="absolute top-2 right-2 p-1 bg-green-500 text-white rounded-full"><Check size={10} /></div>}
                    </button>
                </div>
            </section>
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800 p-2 rounded-2xl">
                    {['small', 'medium', 'large'].map((size) => (
                        <button 
                            ...
                            className={`flex-1 py-3 rounded-xl font-black text-xs uppercase transition-all ${fontSize === size ? 'bg-white dark:bg-slate-700 shadow-md text-green-600' : 'text-slate-400 hover:text-slate-600'}`}
                        >
