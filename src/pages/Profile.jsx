        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-br from-green-600 to-yellow-500 opacity-10"></div>
                
                <div className="w-24 h-24 bg-gradient-to-br from-green-600 to-yellow-500 text-white rounded-[2rem] flex items-center justify-center font-black text-3xl mx-auto mb-6 shadow-lg border-4 border-white relative z-10">
                ...
                <p className="text-green-600 font-black text-sm mb-1">@{profile?.nickname || 'sem_apelido'}</p>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-lg p-8">
                <h3 className="font-black text-slate-800 mb-6 flex items-center gap-2 text-sm">
                    <Award size={18} className="text-green-600" /> {t('next_achievement')}
                </h3>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 md:p-10">
                <h3 className="font-black text-2xl text-slate-800 mb-8 flex items-center gap-3">
                    <Edit2 size={24} className="text-green-600" /> {t('profile_settings')}
                </h3>
                ...
                <form onSubmit={handleSave} className="space-y-6">
                    <div>
                        ...
                        <div className="relative">
                            <span className="absolute left-5 top-1/2 -translate-y-1/2 font-black text-green-600">@</span>
                            <input 
                                type="text" 
                                value={formData.nickname}
                                onChange={(e) => setFormData({ ...formData, nickname: e.target.value.replace(/\s/g, '').toLowerCase() })}
                                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl pl-10 pr-5 py-4 font-black text-slate-800 focus:border-green-500 focus:bg-white outline-none transition-all placeholder:text-slate-300 shadow-sm"
                                placeholder="ex: petrobras_simulador"
                                maxLength={15}
                            />
                        </div>
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-4 font-bold text-slate-800 focus:border-green-500 focus:bg-white outline-none transition-all placeholder:text-slate-300 shadow-sm"
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 md:p-10">
                <h3 className="font-black text-2xl text-slate-800 mb-8 flex items-center gap-3">
                    <PieChart size={24} className="text-green-600" /> {t('stats_numbers')}
                </h3>
                ...
                <div className="mt-8 p-6 bg-green-50/50 rounded-3xl border border-green-100 flex items-center gap-6">
                    <div className="w-20 h-20 bg-white rounded-2xl shadow-sm flex items-center justify-center text-green-600 flex-shrink-0">
                        <Target size={32} />
                    </div>
                    <div>
                        <h4 className="font-black text-green-900 text-lg">{t('acc_precision')}</h4>
                        <p className="text-sm font-bold text-green-700/60 mb-2">{t('acc_precision_desc').replace('{total}', stats.totalQuestions)}</p>
                        <div className="w-full bg-green-100 h-3 rounded-full overflow-hidden">
                            <div className="bg-green-600 h-full rounded-full transition-all duration-1000" style={{ width: `${stats.totalQuestions > 0 ? (stats.totalCorrect / stats.totalQuestions) * 100 : 0}%` }}></div>
                        </div>
                    </div>
                </div>
            </div>
