    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <button 
            onClick={() => navigate('/dashboard')}
            className="text-sm font-bold text-slate-500 hover:text-green-600 flex items-center gap-2 transition-colors"
          >
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-10">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <p className="text-green-500 font-bold text-xs uppercase tracking-widest mb-1">{t('conv_rate')}</p>
            <h2 className="text-3xl font-black text-green-600">
              {stats.total > 0 ? Math.round((stats.premium / stats.total) * 100) : 0}%
            </h2>
          </div>
          <div className="bg-gradient-to-br from-green-600 to-yellow-500 p-6 rounded-3xl text-white shadow-lg flex items-center gap-5">
            <div className="p-4 bg-white/20 rounded-2xl"><Trophy size={32}/></div>
            <div>
              <p className="text-green-100 font-bold text-xs uppercase tracking-widest">{t('active_panel')}</p>
                ...
                <tbody className="divide-y divide-slate-100">
                  {users.map((user) => (
                    <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-6 text-lg font-black text-green-600">{user.bestScore}%</td>
          ...
          <div className="space-y-6">
             <div className="bg-white rounded-3xl border border-slate-200 shadow-xl p-8">
                <h2 className="font-black text-2xl text-slate-800 mb-6 flex items-center gap-3">
                   <MessageSquare className="text-green-600" /> {t('support_requests_title')}
                </h2>
